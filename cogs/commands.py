...
@tree.command(name="register_ign", description="Register a user's IGN")
@app_commands.describe(user="The Discord user to register the IGN for", ign="The in-game name to register")
@commands.has_any_role("Admin", "Mod", "Union Leader")
async def register_ign(self, interaction: discord.Interaction, user: discord.Member, ign: str):
    async with await get_connection() as conn:
        # Check if the user has a union assigned already
        result = await conn.fetchrow("SELECT union_role_id FROM users WHERE discord_id = $1", user.id)
        if not result or not result["union_role_id"]:
            await interaction.response.send_message("❌ This user is not assigned to a union yet.", ephemeral=True)
            return

        union_role_id = result["union_role_id"]

        # Register or update IGN
        await conn.execute(
            """
            INSERT INTO users (discord_id, ign, union_role_id)
            VALUES ($1, $2, $3)
            ON CONFLICT (discord_id)
            DO UPDATE SET ign = EXCLUDED.ign
            """.strip(),
            user.id, ign, union_role_id
        )
