...
(commands.Cog):
    ...
    @app_commands.describe(user="The user to assign", union_role="The union role name")
    async def set_union(self, interaction: discord.Interaction, user: discord.Member, union_role: str):
        if not can_manage_union(interaction.user, union_role):
            await interaction.response.send_message("❌ You don't have permission to assign users to this union.", ephemeral=True)
            return
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET union_name = ? WHERE discord_id = ?", (union_role, str(user.id)))
            cursor.execute("INSERT OR IGNORE INTO users (discord_id, ign, union_name) VALUES (?, '', ?)", (str(user.id), union_role))
            conn.commit()
            conn.close()

            role = discord.utils.get(interaction.guild.roles, name=union_role)
            if role:
                await user.add_roles(role)
            await interaction.response.send_message(f"✅ {user.mention} has been added to `{union_role}`.")
        except Exception as e:
            print(f"DB ERROR: {e}")
            await interaction.response.send_message("⚠️ Failed to assign union.", ephemeral=True)
    ...
    async def unset_union(self, interaction: discord.Interaction, user: discord.Member):
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("SELECT union_name FROM users WHERE discord_id = ?", (str(user.id),))
            result = cursor.fetchone()
            if not result or not result[0]:
                await interaction.response.send_message("ℹ️ User is not assigned to any union.")
                return
            union_name = result[0]
            if not can_manage_union(interaction.user, union_name):
                await interaction.response.send_message("❌ You don't have permission to remove this user from their union.", ephemeral=True)
                return
            ...
            await interaction.response.send_message(f"✅ {user.mention} has been removed from `{union_name}`.")
        except Exception as e:
            print(f"DB ERROR: {e}")
            await interaction.response.send_message("⚠️ Failed to remove union.", ephemeral=True)

    @app_commands.command(name="appoint_union_leader", description="Assign a leader to a union role")
    @app_commands.describe(union_role="The union role", user="The new leader")
    async def appoint_union_leader(self, interaction: discord.Interaction, union_role: str, user: discord.Member):
        if not is_manager(interaction.user):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("INSERT OR REPLACE INTO union_leaders (role_name, leader_id) VALUES (?, ?)", (union_role, str(user.id)))
            conn.commit()
            conn.close()
            await interaction.response.send_message(f"👑 {user.mention} is now the leader of `{union_role}`.")
        except Exception as e:
            print(f"DB ERROR: {e}")
            await interaction.response.send_message("⚠️ Failed to assign union leader.", ephemeral=True)

    @app_commands.command(name="dismiss_union_leader", description="Remove the leader of a union")
    @app_commands.describe(union_role="The union role to clear")
    async def dismiss_union_leader(self, interaction: discord.Interaction, union_role: str):
        if not is_manager(interaction.user):
            await interaction.response.send_message("❌ Only Admin or Mod can remove leaders.", ephemeral=True)
            return
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("DELETE FROM union_leaders WHERE role_name = ?", (union_role,))
            conn.commit()
            conn.close()
            await interaction.response.send_message(f"🗑️ The leader of `{union_role}` has been removed.")
        except Exception as e:
            print(f"DB ERROR: {e}")
            await interaction.response.send_message("⚠️ Failed to remove union leader.", ephemeral=True)

    @app_commands.command(name="list_union_leaders", description="Show all union leaders")
    async def list_union_leaders(self, interaction: discord.Interaction):
        try:
            ...
            if not rows:
                ...
                return
            text = "\n".join([f"👑 `{role}` → <@{leader_id}>" for role, leader_id in rows])
            await interaction.response.send_message(f"📋 Union Leaders:\n{text}")
        except Exception as e:
            print(f"DB ERROR: {e}")
            await interaction.response.send_message("⚠️ Failed to list union leaders.", ephemeral=True)
